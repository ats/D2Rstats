...
dateGE <- "2024-01-01"             # date in YYYY-mm-dd
                                  # will filter to activity date >= to this
# date_before = lubridate::today()
date_before <- "2021-01-31"
...
tbljson |> unnest_wider(..JSON) |> select(period, activityDetails, entries) |>
  ...
  if(date_before != "") {
    temptbl |> filter(period <= date_before) ->
      temptbl
  }
...
tic()
  ...
  unnest_wider(values, names_repair = "universal") |>
    select(-standing...194) |>
    rename(standing = standing...7) |>
...
```
```{r}
weapon_barplot(levelWeapon, mode=4, ncol = 3, title = "Weapons used in raids") + weapon_treemap(levelWeapon, mode=4)
```
...
if("Iron Banner" %in% pvp_games$displayProperties.name.x) {
  pvp3 <- pvp_games |>
    filter(grepl("Iron Banner", displayProperties.name.x)) |>
...
#| eval: true
#| echo: true
# Iron Banner, Heresy Act I

iron_banner <- pvp_games |>
  filter(date >= as.Date("2025-02-01") &
         date < as.Date("2025-02-28") &
         grepl("Iron Banner", displayProperties.name.x)
         ) 

iron_banner |> ungroup() |> summarise(n=n(), .by=c("standing", "characterClass"))
...
iron_banner |>
  ...
  theme_solarized_2(light=FALSE, base_family = font_family) +
  theme(legend.position = "none") +
  labs(title = "Iron Banner - Heresy Act I",
       caption = "Wins & Losses / Scaled by player score",
       y = "KDA", x=NULL)
}
...
# limited to the first IB week of Episode Echoes (S24) 2024,
...
pvp_games |>
  ...
  theme_solarized_2(light=FALSE, base_family = font_family) +
  theme(legend.position = "none") +
  labs(title = "Iron Banner #1, Season 24 (Episode: Echoes)",
       caption = "Wins & Losses / Scaled by player score",
       subtitle = "The road to Gilded x 7, all games on Hunter",
       y = "KDA", x=NULL)
```
```{r}
#| eval: false
...
# another variation on IB info; this will show data by character for the first
# IB of season 23
...
pvp_games |>
  ...
  ggplot(aes(x=date, y=as.numeric(kda), color=standing, size=score, shape=characterClass)) +
    geom_hline(yintercept = 1, color="white") +
    geom_quasirandom( varwidth = TRUE, width=.3, orientation = "x", alpha=.7) +
  theme_solarized_2(light=FALSE, base_family = font_family) +
  theme(legend.position = "none") +
  labs(title = "Iron Banner #1, Season 23",
       subtitle = "Wins & Losses / Scaled by player score",
       caption = "The road to Gilded x 6",
       y = "KDA", x=NULL)
