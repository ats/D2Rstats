...
# here pipe the exploded data into a function that will complete the character information
# by 1) checking stored data set for character; 2) API fetching character data if not found;
# 3) storing retrieved data to local data set
  
  # manage_characters <- function(.data) {
  #   # set up character data set if needed
  #   char_file <- glue::glue("{membershipId}-data/characters_data.csv")
  #   if(fs::file_exists(char_file)) {
  #     load(char_file)
  #   } else {
      # we need to make first build of character data
      # this should make the set of unique characters found
      characters_data <-
    ...
    # for each unique characterId, check saved characters for match
  ...
  # mutate(pgcr_membership_id =
  #   membershipId) |> select(-membershipId) |> filter(pgcr_membership_id == membershipId) |>
  # unnest_auto(characterId)
...
levelWeapon |>
 mutate(date = lubridate::date(period)) |> 
  ...
  arrange(desc(period)) |> head(n=50) |> select(-instanceId, -period) |>
